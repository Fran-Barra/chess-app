...
class Game (public val board: Board,
    ...
    fun makeMovement(player: Player, origin: Vector, destination: Vector): GameMovementResult {
        ...
        if (pieceR.isFailure) return manageFailure(pieceR, "board", "get piece in position")
        ...
        val movementStrategy: MovementStrategy = pieceMovementStrategy[piece.getPieceType()]
            ?: return MovementFailed("Piece type don't match with the rules of the game")

        if (!movementStrategy.checkMovement(pieceEatingRuler, player, origin, destination, board))
            return MovementFailed("Movement is not valid for this piece")
